 ...
 * ─────────────────────────────────────────────────────────────────
 * Flow:
 *   LinkedIn Email → Zapier Mail → This API → Claude reads PDF
 *   → Evaluates candidate → ATS Dashboard with resume viewer
 * ─────────────────────────────────────────────────────────────────
 */
...
function parseLinkedInEmail(subject, rawBody) {
  ...
  const r3 = subject.match(/applicant.*?for\s+(.+?)(?:\s*[-–]\s*|\s*$)/i);
  if (r1) roleApplied = r1[1].trim();
  else if (r2) roleApplied = r2[1].trim();
  else if (r3) roleApplied = r3[1].trim();
...
app.post("/webhook/parseur", async (req, res) => {
  try {
    ...
    if (payload.resume_file_data && payload.resume_file_data.length > 500) {
      try {
        const cleaned = payload.resume_file_data.replace(/^data:application\/pdf;base64,/, "");
        resumeBuffer  = Buffer.from(cleaned, "base64");
        resumeBase64  = cleaned;
        resumeSource  = "file_data";
        console.log("📄 PDF from file_data field, size:", resumeBuffer.length);
      } catch(e) {
        console.log("⚠️ Could not parse file_data:", e.message);
      }
    }

    // Option B — download from URL
    if (!resumeBuffer && payload.resume_url) {
      try {
        resumeBuffer = await downloadPDF(payload.resume_url);
        resumeBase64 = resumeBuffer.toString("base64");
        resumeSource = "url";
        console.log("📄 PDF downloaded from URL, size:", resumeBuffer.length);
      } catch(e) {
    ...
    // Store PDF buffer separately (not in candidate object to save memory)
    if (resumeBuffer) {
      store.resumes[candidateId] = {
        buffer:   resumeBuffer,
        base64:   resumeBase64,
        filename: filename,
    ...
    const evaluation  = await evaluateCandidate(rawCandidate, resumeBase64, matchedRole);
...
// ═══════════════════════════════════════════════════════════════════
//  ROUTE 2 — SERVE RESUME PDF (inline viewer)
//  Dashboard calls GET /candidates/:id/resume to display PDF
// ═══════════════════════════════════════════════════════════════════
app.get("/candidates/:id/resume", (req, res) => {
  const resume = store.resumes[req.params.id];
  if (!resume) {
...
// ═══════════════════════════════════════════════════════════════════
//  ROUTE 3 — DOWNLOAD RESUME PDF
// ═══════════════════════════════════════════════════════════════════
app.get("/candidates/:id/resume/download", (req, res) => {
  const resume = store.resumes[req.params.id];
  if (!resume) {
    return res.status(404).json({ error: "Resume not found" });
  }
...
app.get("/health", (req, res) => res.json({
  ...
  resumes: Object.keys(store.resumes).length,
...
//  CLAUDE EVALUATION
// ═══════════════════════════════════════════════════════════════════
async function evaluateCandidate(candidate, resumeBase64, role) {
  const isCreative = role?.type === "creative";
  const hasPDF     = !!resumeBase64;

  console.log(`🤖 Evaluating ${candidate.full_name} | PDF: ${hasPDF}`);
...
${hasPDF ? "- Full resume PDF attached above — read carefully for complete evaluation." : "- No resume content available — evaluate based on available information only."}
...
Return ONLY valid JSON:
{
  ...
  "red_flags": [],
  "notes": "<notes for hiring manager>"
}

SCORING: 80-100 advance · 60-79 worth a look · 40-59 needs review · 0-39 does not qualify
LUXURY FIT: Experience with luxury, premium, or cultural brands aligned with art, culture, travel, fashion.
`.trim();

  const messages = hasPDF ? [{
    role: "user",
    content: [
  ...
  try {
    const response = await anthropic.messages.create({
      model: CONFIG.MODEL, max_tokens: 1000, messages,
    });
    ...
    return {
      ...
      summary: "Resume PDF received. Please review manually.",
      strengths: ["Resume attached — manual review recommended"],
      concerns:  ["Automated evaluation incomplete"],
      experience_score: 45, skills_score: 45, culture_fit_score: 45,
      portfolio_score: null, luxury_brand_fit: "medium",
      interview_questions: [
        "Walk us through your experience for this role.",
        "What draws you to Assouline?",
        "How does your background align with luxury publishing?"
      ],
      red_flags: [],
      notes: `PDF: ${candidate.resume_file}. Error: ${err.message}`,
    };
  }
}
...
app.listen(CONFIG.PORT, () => {
  ...
  console.log(`📄 Resume:   GET  /candidates/:id/resume`);
  console.log(`📊 List:     GET  /candidates`);
